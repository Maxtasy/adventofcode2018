...
def flood(clay, max_y):
	...
	def scan(x, y, dx):
		# Walks from x in direction dx, resolving each column below as it
		# goes, until it hits a clay wall (contained) or an unsupported
		# drop-off (open, water flows away).
		cx = x
		while True:
			nx = cx + dx
			if (nx, y) in clay:
...
def part1(input_file):
	...
	return sum(1 for (x, y), v in tiles.items() if min_y <= y <= max_y)
